...
function getMovies(category,page="1")
{
    ...
    req.open("GET",`https://api.themoviedb.org/3/movie/${category}?api_key=52c5e9e35fdf713465df72a1b8524c13&language=en-US&page=${page}`);
    req.send();
...
function searchWord(searchword)
{
    ...
    let req = new XMLHttpRequest();
    req.open("GET",`https://api.themoviedb.org/3/search/movie?api_key=52c5e9e35fdf713465df72a1b8524c13&language=en-US&query=${searchword}&page=1&include_adult=false`);
    req.send();
    req.onreadystatechange=function(){
        if(req.readyState == 4 && req.status == 200)
        {
